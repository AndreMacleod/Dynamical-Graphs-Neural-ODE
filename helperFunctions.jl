...
# function to return indeces of k nearest vertices to v 
function nearest_vertices(M, v, k)
    #distances = []  # preallocate sizes
    vertex_norm_distances = Tuple[]
...
# function that returns disatnce between vertex and another vertex, given its index
function vertex_distances(M, v, index)
    distances = [] 
    for j in 1:size(M)[1]                                          # iterating over all columns in matrix
        push!(distances, M[j, v] - M[j, index])
    end
    return distances
end
...
function vertex_distances_1(M, v, k)
    distances = []  # preallocate sizes
